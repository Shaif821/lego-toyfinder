<template>
  ...
  >
    ...
    >
      ...
      <swiper-slide
        v-if="this.$store.state.slideState !== 4"
        class="swiper__slide--no-swiping"
      >
        <transition
          v-if="this.$store.state.slideState === 1"
        ...
        >
          <ScreenSaver
            class="swiper__slide--no-swiping"
            @click.native="toSurvey()"
          ></ScreenSaver>
        </transition>
        <transition
          v-else-if="this.$store.state.slideState === 2"
          leave-active-class="animated fadeOut"
          enter-active-class="animated fadeIn"
          mode="out-in"
        >
        ...
        <transition
          v-else-if="(this.$store.state.slideState = 3)"
          leave-active-class="animated fadeOut"
          enter-active-class="animated fadeIn"
          mode="out-in"
        >
...
<script>
import ScreenLoader from "./layout/ScreenLoader.vue";
import ProductList from "./product/ProductList.vue";
import ScreenSaver from "./layout/ScreenSaver.vue";
import LegoSurvey from "./survey/LegoSurvey.vue";
import CardBoard from "./layout/CardBoard.vue";
...
export default {
  ...
  methods: {
    moveSlide(time) {
      //Deze functie zorgt ervoor dat de volgende slide verschijnt
      this.$nextTick(() => {
    ...
    toSurvey() {
      if (this.isFullScreen) {
        //Als de pagina in fullscreen weergegeven wordt
        this.$nextTick(() => {
    ...
    fullScreen() {
      //Deze functie zorgt ervoor dat, zodra er geklikt wordt op de scherm, de pagina in volledige weergave wordt getoond.
