<template>
  <div
    class="sharelist-container swiper-no-swiping"
    :class="[
      test
  ...
  >
    <div
      class="sharelist-container__section-1"
      :class="{ 'move-close-button': writeMail }"
    >
      <transition
      ...
      >
        <img
          ...
          @click="closeShareList()"
        />
        <img
          v-else
          class="animated pulse infinite"
          src="../../assets/images/layout/close-copy.png"
          @click="(writeMail = false), (mailResponse = false)"
        />
      </transition>
    </div>
    ...
    <div class="sharelist-container__section-3">
      <div
        class="section-3-row"
        :class="[!writeMail ? 'section-3-row' : 'section-3-row-out']"
      >
        ...
        <p class="section-3-row__bottom-title">
          Scan deze QR-code met je telefoon en deel de link met familie en
          vrienden.
        </p>
      </div>
      <div
        :class="[
          !writeMail ? 'section-3-row__border' : 'section-3-row__border-out'
        ]"
        class="section-3-row__border"
      >
        <p :class="{ 'border__text-out': writeMail }" class="border__text">
          OF
        </p>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <input
              ...
              name="email_field"
              type="text"
              class="section-3-row__middle section-3-row__input"
              placeholder="Mijn emailadres"
            />
            <div v-else :key="2">
              <br />
              <img
                src="../../assets/images/layout/loader.png"
                class="rotating"
              />
            </div>
          </transition>
          ...
          >
            <p
              ...
              :key="1"
              class="section-3-row__bottom-title"
            >
              Voer een geldige emailadres in
            </p>
            <p v-else :key="2" class="section-3-row__bottom-title">
              Vul hierboven jouw emailadres is en druk op verstuur en ontvang
              direct jouw lijstje in je mailbox.
            </p>
          </transition>
        </form>
        ...
        >
          ...
          <div v-else>
            ...
            <img
              class="section-3-row__image section-3-row__image--down"
              src="../../assets/images/layout/thumb.png"
            />
            <p class="section-3-row__bottom-title">
    ...
    <div
      class="sharelist-container__lego-image"
      :class="{ 'sharelist-container__lego-image-out': writeMail }"
    >
      <img
        class="lego-image__image"
        src="../../assets/images/layout/lego-share.png"
      />
    </div>

    <div class="sharelist-container__section-4" @click="newWishList()">
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          axios
            ...
            .then(response => {
              if (response) {
                (this.isSending = false), (this.userMail = "");
