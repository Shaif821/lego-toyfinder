<template>
  <div
    :class="[
      { 'wishlist__container--open': isActiveWishList },
      { 'wishlist__container--close': notActiveWishList }
    ]"
    class="wishlist__container"
  >
    <div
      style="cursor: pointer;"
      class="wishlist__clipboard__container"
      @click="openWishList()"
    >
      <div class="wishlist__clipboard">
        ...
        >
          <img
            ...
            :key="1"
            class="wishlist__star"
            src="../../assets/images/layout/star-icon-mini.png"
          />
          <img
            v-else
            :key="2"
            class="wishlist__star wishlist__star--un"
    ...
    <div
      class="wishlist__phone__container"
      :class="{ add__shadow: isActiveWishList }"
    >
      <div class="wishlist__phone__header">
        <img
          class="wishlist__phone__title__img"
      ...
      <div class="wishlist__phone">
        <img
          class="wishlist__phone__legohead"
          src="../../assets/images/layout/lego-head.png"
        />
        <div class="phone_contents">
          <div v-for="(i, index) in 5" :key="index">
            <transition
              ...
              class="contain_favorite_text"
              mode="out-in"
            >
              <span
                v-if="$store.state.favorites[i - 1]"
                :key="$store.state.favorites[i - 1]['Product Name NL']"
                class="is_favorited"
              >
                ...
                <img
                  ...
                  @click="removeFavorited($store.state.favorites[i - 1])"
                />
              </span>
              ...
              <span v-else :key="$store.state.favorites[i - 1]">
        ...
        <div
          class="wishlist_share"
          style="cursor: pointer;"
          :class="[
            $store.state.favorites.length > 0 ? 'favorited' : 'un_favorited'
          ]"
        >
          <p v-if="$store.state.favorites.length > 0" @click="openShareList()">
...
.wishlist__phone__container {
  ...
  right: 0;
  margin-left: 89px;
  background: white url("../../assets/images/layout/border-left.png") left
    no-repeat;
  display: flex;
...
.favorited {
  transition: 0.3s ease-in-out;
  background: #ffc600 url("../../assets/images/layout/border-left.png") left
    no-repeat;
...
.un_favorited {
  transition: 0.3s ease-in-out;
  background: #496078 url("../../assets/images/layout/border-left.png") left
    no-repeat;
