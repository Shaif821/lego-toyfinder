<template>
  <div
    :class="[
      this.$store.state.transitionSlide ? 'survey__hide' : 'survey__show'
    ]"
  >
    <div class="survey__container">
      <div
        :class="[{ hide__interest: $store.state.transitionSlide }]"
      ...
      >
        ...
        >
          ...
          >
            >
            <img
              class="survey__header--img animated pulse infinite"
      ...
      <transition
        v-if="view !== 'SurveyTheme'"
        enter-active-class="animated slideInRight slowDown"
      ...
      <transition
        v-else
        enter-active-class="animated slideInRight slowDown--down"
        enter
        mode="out-in"
      >
        <component
...
<script>
import SurveyAge from "./SurveyAge.vue";
import SurveyInterest from "./SurveyInterest.vue";
import SurveyTheme from "./SurveyTheme.vue";
...
export default {
  ...
  props: {
    indexAnimation: { type: Number, default: 0 },
    resetApp: { type: Boolean, required: true }
  },
  ...
  watch: {
    checkThemeChange() {
      if (this.$store.state.isActiveTheme) {
        //Wanneer de gebruiker meteen naar de thema's toe wilt (vanuit de screensaver)
  ...
  methods: {
    changeView(choice) {
      //Als de gebruiker vanuit het begin scherm een keuze maakt, of vanuit de producten slide
      this.$store.state.currentSurvey
        ? this.addChoice(choice, true)
        : this.saveChoice(choice);
    },

    saveChoice(choice) {
      //Sla de keuze op en ga door naar de volgende slide of view
      this.addChoice(choice, false);
      if (this.view === "SurveyInterest" || this.view === "SurveyTheme") {
        //Ga door naar de volgende slide(ScreenLoader)
        this.nextSlide();
      } else {
    ...
    addChoice(choice, loadProduct) {
      //Check welke view het is en sla het antwoord bij de bijbehorende data op
      switch (this.view) {
        case "SurveyAge":
      ...
      if (loadProduct) {
        //Ga door naar de next slide (Als je dus vanuit de producten pagina komt)
        this.nextSlide();
      }
    },

    nextSlide() {
      //Slaat de keuzes op in de store & verander de state van de slider
    ...
    watchView() {
      if (this.$store.state.currentSurvey === "SurveyAge") {
        //(Vanuit de producten pagina) Checkt welke vieuw in currentSurvey zit
        this.view = "SurveyAge";
